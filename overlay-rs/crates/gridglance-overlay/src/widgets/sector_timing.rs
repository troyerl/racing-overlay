...
fn paint_sector_cell(
    ...
    let rad = if elegant {
        (rect.height() * 0.28).clamp(4.0, 8.0)
    } else {
        cell_radius(rect.height())
    };
    ...
    if elegant {
        ...
        label(
            ...
            &line,
            (rect.height() * 0.42 * scale).clamp(10.0, 14.0),
            sec_text,
            true,
        );
        if show_delta {
            if let Some(d) = cell.delta.filter(|d| d.abs() >= 0.005) {
                ...
                label(
                    ui,
                    Pos2::new(rect.center().x, rect.bottom() - rect.height() * 0.22),
                    Align2::CENTER_CENTER,
                    &signed_delta(d, 2),
                    (rect.height() * 0.28 * scale).clamp(8.0, 11.0),
                    dc,
                    false,
                );
            }
...
pub fn paint(ui: &mut Ui, ctx: &mut WidgetCtx<'_>) {
    ...
    let pad = if elegant {
        (card.height() * 0.04).max(5.0)
    } else {
        panel_pad(card.height())
    };
    let h = card.height();
    let scale = ctx.cfg.text_scale(SECTION);
    ...
    label(
        ...
        (cur_h * (if elegant { 0.68 } else { 0.72 }) * scale).clamp(14.0, 32.0),
        ctx.cfg.color(SECTION, "text", "#f4f6f8"),
        true,
    );
    ...
    // Both styles: single-line LAST / BEST (no stacked columns).
    if elegant {
    ...
    let top = sub.bottom() + if elegant { 4.0 } else { h * 0.04 };
    let avail = (card.bottom() - pad - top).max(18.0);
    // Elegant: cap sector strip height — don't stretch to fill the card.
    let ch = if elegant {
        let want = if show_delta { 28.0 } else { 22.0 };
        avail.min(want)
    } else {
        avail.max(24.0)
    };
    let gap = iw * (if elegant { 0.025 } else { 0.03 });
    ...
    for (i, cell) in sectors.iter().enumerate() {
        ...
        paint_sector_cell(ui, ctx, cell_rect, i + 1, cell, show_delta, elegant);
        x += cw + gap;
    }
}
