...
pub fn paint(ui: &mut Ui, ctx: &mut WidgetCtx<'_>) {
    ...
    if only_actionable && !advice.actionable && !ctx.edit_mode {
        // Keep a transparent empty panel so layout stays; draw nothing.
        let _ = full_rect(ui);
        return;
    }
    ...
    if ctx.cfg.bool_key(SECTION, "show_title", true) {
        let hh = (card.height() * 0.14).max(20.0);
        let title = ctx.cfg.str_key(SECTION, "title", "PIT ENGINEER");
        draw_section_header(
            ui,
            ctx.cfg,
            SECTION,
            Rect::from_min_size(Pos2::new(card.left() + pad, y), Vec2::new(text_w, hh)),
            &title,
            radius,
        );
        y += hh + 6.0;
    ...
    let chip_h = (card.height() * 0.18).max(22.0);
    ...
    ui.painter()
        .rect_filled(chip, CornerRadius::same(6), chip_bg);
    label(
        ui,
        chip.center(),
        Align2::CENTER_CENTER,
        &label_txt,
        (chip_h * 0.45).clamp(11.0, 18.0),
        Color32::WHITE,
        true,
    );
    y += chip_h + 8.0;
